...
class ContentFilterService {
  ...
  static fixHTMLTags(html) {
    ...
    // 修復中文標籤名稱
    result = result.replace(/<強調/g, '<strong>');
    result = result.replace(/<\/強調>/g, '</strong>');
    result = result.replace(/<粗體/g, '<strong>');
    result = result.replace(/<\/粗體>/g, '</strong>');
    result = result.replace(/<斜體/g, '<em>');
    result = result.replace(/<\/斜體>/g, '</em>');
  ...
  static quickCheck(text) {
    ...
    if (!text) {
      return { passed: false, issues: ['內容為空'] };
    }
    ...
    if (foundSimplified.length > 0) {
      issues.push(`檢測到簡體字: ${foundSimplified.slice(0, 5).join(', ')}...`);
    }
      ...
      if (matches && matches.length > 0) {
        matches.forEach(match => {
          found.push({
            ...
            text: match,
            severity: 'warning',
            suggestion: '此句型為機械式模板，建議改寫為更自然的表達方式'
          });
        });
      }
    });
      ...
      if (matches && matches.length > 0) {
        matches.forEach(match => {
          found.push({
            ...
            text: match,
            severity: 'error',
            suggestion: '此為不可追溯的虛假聲稱，必須移除或改寫為可追溯的引用'
          });
        });
      }
    });
  ...
  static validateKeywordDensity(text, keyword) {
    ...
    // 判斷是否過高
    const isPassed = density <= 0.015; // 1.5% 為上限
    const isWarning = density > 0.012; // 1.2% 以上警告
    ...
    return {
      ...
      message: isPassed 
        ? `關鍵字密度正常 (${densityPercent}%)` 
        : `⚠️ 關鍵字密度過高 (${densityPercent}%)，可能被視為 Keyword Stuffing`
    };
  }
      ...
      if (matches && matches.length > 0) {
        matches.forEach(match => {
          issues.push({
            ...
            text: match,
            severity: 'error',
            suggestion: '必須提供具體來源名稱、年份或連結，例如：「根據美國XX協會(2023)研究顯示」'
          });
        });
      }
    });
